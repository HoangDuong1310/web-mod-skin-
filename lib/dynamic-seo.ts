...
export async function getSEOSettings(): Promise<SEOSettings> {
  try {
    ...
    return {
      ...
      siteDescription:
        settings.siteDescription || DEFAULT_CONFIG.siteDescription,
      siteUrl,
...
export async function generateDynamicMetadata(
  ...
  const siteUrl = (settings.siteUrl || DEFAULT_CONFIG.siteUrl).replace(
    /\/$/,
    ''
  )
  ...
  return {
    ...
    openGraph: {
      ...
      images: [
        {
          url: imageUrl || `${siteUrl}/images/img.jpg`,
          width: 1200,
          height: 630,
    ...
    twitter: {
      card: (settings.twitterCard || 'summary_large_image') as
        | 'summary'
        | 'summary_large_image',
      title: fullTitle,
      description,
      images: [imageUrl || `${siteUrl}/images/img.jpg`],
    },
    alternates: {
      canonical: fullUrl,
    },
    ...
    other: {
      ...(settings.googleSearchConsoleId && {
        'google-site-verification': settings.googleSearchConsoleId.replace(
